...
Similarly, the following code performs all pairwise tests for all Ontologies and GO levels:
```{r, eval=FALSE}
allOncoResults <- equivTestSorensen(allOncoGeneLists, 
                                    geneUniverse = humanEntrezIDs,
                                    orgPackg = "org.Hs.eg.db")
class(allOncoResults)
...
```
The visualization of  these results is not good due to the huge number of  obtained data, for this reason they were saved in the available file "allOncoResults.rda" using  `save(allOncoResults, file = "allOncoResults.rda")`. If the user wishes to see these results, he can easily load them using `load("allOncoResults.rda")`. In addition, files: "summary_goSorensen.xlsx", "AllResults_Test.txt" and "AllResults_Tables.txt" are also provided with a summary of these results.
