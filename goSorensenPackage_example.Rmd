...
package: goSorensen
abstract: >
 This document provides an example for the manipulation of the `goSorensen` R Package, using a compilation of feature lists  coming from true genomic experiments.
...
and the same for other test result fields: standard error `getSE`, upper bound equivalence limit `getUpper`, initially planned number of bootstrap resamples `getNboot` and number of effective bootstrap resamples `getEffNboot` (sometimes, the computation of the test statistic may produce a non-finite result). The last two functions are only available for bootstrap tests.
...
For objects of class "list", all these functions (`equivTestSorensen`, `dSorensen`,  `seSorensen`, `duppSorensen`) assume a list of character vectors containing gene identifiers and all pairwise computations are performed. But this approach is much slower than the previous one as the contingency tables must be previously obtained.
...
# results. Instead of running this line, we suggest loading its result which 
